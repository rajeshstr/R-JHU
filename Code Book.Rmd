...
The run_analysis.R script performs the data preparation and then followed by the 5 steps required as described in the course project’s definition.
   ...
   * Merges the training and the test sets to create one data set
       ...
       y_data (10299 rows, 1 column) is created by merging y_train and y_test using rbind() function
        ...
        final_data (10299 rows, 563 column) is created by merging Subject, Y and X using cbind() function

  * Extracts only the measurements on the mean and standard deviation for each measurement
        tidy_data (10299 rows, 88 columns) is created by subsetting final_data, selecting only columns: subject, code and the measurements on the mean and standard deviation (std) for each measurement
